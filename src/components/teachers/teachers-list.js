...
import TeachersListItem from "./teachers-list-item";
...
const TeachersList = () =>{
    ...
    Options.exportMenu= [{
        ...
        exportFunc: async (cols, datas) => {
            let res=await getFetchWithPropsPlain('/teachers',user,'Please wait... Fetching Data.')
            ExportPdf(cols, res.data, 'Teachers List')
        }
    }, {
        label: 'Export CSV',
        exportFunc: async (cols, datas) => {
            let res=await getFetchWithPropsPlain('/teachers',user,'Please wait... Fetching Data.')
            ExportCsv(cols,res.data , 'Teachers List')
        }
    }]
    ...
    const columns = [
        ...
        { title: "Teacher UID", field: "id", resizeable:true},
                                ...
                                .then((result) => {
                                    resolve({
                                        data: result.data.filter(t=>{
                                            return (t.firstName?.toLowerCase().includes(query.search?.toLowerCase())
                                            ||t.lastName?.toLowerCase().includes(query.search?.toLowerCase())
                                            || t.idNumber?.toLowerCase().includes(query.search?.toLowerCase())
                                            || t.address?.toLowerCase().includes(query.search?.toLowerCase())
                                            || t.email?.toLowerCase().includes(query.search?.toLowerCase())
                                            || t.phone?.toLowerCase().includes(query.search?.toLowerCase()))
                                        }),
